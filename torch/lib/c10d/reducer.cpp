  ...
  // can be marked as ready for reduction.
  {
    ...
    for (size_t replica_index = 0; replica_index < replica_count;
      ...
      for (size_t variable_index = 0; variable_index < variable_count;
        ...
        // grad_accumulator执行完后，autograd_hook就会运行
        hooks_.emplace_back(
...
void Reducer::mark_variable_ready(VariableIndex index) {
  ...
  //检查桶里的变量是不是都ready了，如果没有东西pending,那就是都ready了
...
void Reducer::mark_bucket_ready(size_t bucket_index) {
  ...
  // - have kicked off reduction for all buckets, or（所有桶都在allreduce, 那就等着）
  // - found a bucket that's not yet ready for reduction.，还有桶没好，那也等着
  for (; next_bucket_ < buckets_.size() && buckets_[next_bucket_].pending == 0;
    ...
    if (comm_hook_ == nullptr) {
      ...
      //注册了comm_hook那就先跑hook
      //需要注意的是，这个comm_hook 只是处理通信的底层hook,如果想在reduce前分别进行梯度裁剪，
      //还是需要在autograph挂hook
