...
#用户不需要手动对模型参数dtype转换，amp会自动为算子选择合适的数值精度

#autocast可以作为 Python 上下文管理器和装饰器来使用，用来指定脚本中某个区域、或者某些函数，
# 按照自动混合精度来运行。混合精度在操作的时候，是先将 FP32 的模型的参数拷贝一份，
# 拷贝的参数转换成 FP16，而 amp 规定了的 FP16 的算子（例如卷积、全连接），对 FP16 的数值进行操作；
# FP32 的算子（例如涉及 reduction 的算子，BatchNormalize，softmax...），输入和输出是 FP16，计算的精度是 FP32。
# 在反向传播时，依然是混合精度计算，得到数值精度为 FP16 的梯度。
# 最后，由于 GPU 中的 Tensor Core 天然支持 FP16 乘积的结果与 FP32 的累加（Tensor Core math），
# 优化器的操作是利用 FP16 的梯度对 FP32 的参数进行更新。
