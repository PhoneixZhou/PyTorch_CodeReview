...
#对于反向传播的时候，FP16的梯度数值溢出问题，amp提供了梯度scaling操作，
#而且在优化器更新参数前，会自动对梯度unscaling, 所以对用于模型优化的超参数不会有任何影响
#amp 动态scale factor的训练过程
#1. 维护一个FP32数值精度模型的副本
#2. 初始化 s
#3. 在每个iteration:
#   a.拷贝并且转换成FP16模型;
...
#   d.反向传播（FP16的模型参数和参数梯度）
#   e.检查有没有inf 或这nan 的参数梯度
      #如果有：
          #降低s, 回到步骤a
#   f.参数梯度乘1/s
#   g.利用FP16的梯度更新FP32的模型参数
class GradScaler(object):
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    #对outputs 乘scale factor, 并返回，如果enabled=False就原样返回
    ...
    #1.对梯度unscale, 如果之前没有手动调用unscale方法的话;
    #2.检查梯度溢出，如果没有nan/inf,就执行optimizer 的step,如果有就跳过
    ...
    #update方法在每个 iteration 结束前都需要调用，如果参数更新跳过，
    # 会给 scale factor 乘backoff_factor，或者到了该增长的 iteration，
    # 就给 scale factor 乘growth_factor。也可以用new_scale直接更新 scale factor。
