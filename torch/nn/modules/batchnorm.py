...
class _NormBase(Module):
    ...
    num_features: int
    eps: float
    #默认0.1, 更新running_mean, running_var时的动量
    momentum: float
    #默认为True, 训练weight 和bias;否则不更新它们的值
    affine: bool
    #默认为True,是否统计running_mean, running_var
    ...
    def __init__(
        ...
        #weight: 公式中的gamma,初始化为全1 tensor
        #bias: 公式中的beta, 初始化为全0 tensor
        ...
        if self.track_running_stats:
            ...
            #训练时统计输入的mean, 之后用于inference
            self.register_buffer('running_mean', torch.zeros(num_features))
            #训练时统计输入的var,之后用与inference
            self.register_buffer('running_var', torch.ones(num_features))
            #当momentum设置为None时，使用num_batches_tracked计算每一轮更新的动量
    ...
    #避免BC-breaking.
    #在 PyTorch 的开发过程中，Normalization layers 在某个新版本中 引入了 num_batches_tracked 这个 key，
    # 给 BN 记录训练过程中经历的 batch 数，为了兼容旧版本训练的模型，PyTorch 修改了 _version，并修改了 _load_from_state_dict
