...
#实现了 3 个通用的 hook 注册函数，用于注册被应用于全局的 hook。
#这 3 个函数会将 hook 分别注册进 3 个全局的 OrderedDict，使得所有的 nn.Module 的子类实例在运行的时候都会触发这些 hook。
...
class Module:
    ...
    def __init__(self):
        ...
        self.training = True #控制training/testing 状态
        self._parameters = OrderedDict() #在训练过程中会随着BP而更新的参数
        self._buffers = OrderedDict() #在训练过程中不会随着BP而更新的参数
        self._non_persistent_buffers_set = set()
        self._backward_hooks = OrderedDict()#Backward 完成后会被调用的hook
        self._is_full_backward_hook = None
        self._forward_hooks = OrderedDict()#Forward 完成后会被调用的hook
        self._forward_pre_hooks = OrderedDict()#Froward 前会被调用的hook
        self._state_dict_hooks = OrderedDict()#得到state_dict 以后会被调用的hook
        self._load_state_dict_pre_hooks = OrderedDict()#load state_dict前会被调用的hook
        self._modules = OrderedDict()#子神经网络模块

    forward: Callable[..., Any] = _forward_unimplemented

    #属性设置
    #这 3 个函数都会先检查 self.__dict__ 中是否包含对应的属性字典以确保 
    # nn.Module 被正确初始化，
    # 然后检查属性的 name 是否合法，
    # 如不为空 string 且不包含“.”，
    # 同时还会检查他们是否已经存在于要修改的属性字典中。

    #增加不通过BP更新的buffer
    #如果buffer不是persistant的，还会同时更新到self._non_persistent_buffers_set中
    ...
    #apply 和 _apply 的区别：_apply()是专门针对parameter 和buffer 而实现的一个
    # “仅供内部使用”的接口，单数apply 函数是“公有接口” 
    #1.通过self.children()进行递归的调用;
    #2.对self._parameters中参数及其gradient通过function进行处理;
    #3.对sef._buffers中buffer逐个通过function来进行处理;
    def _apply(self, fn):
        #对子模块进行递归调用
        for module in self.children():
            module._apply(fn)

        #为了BC-breaking 而新增了一个tensor类型判断
    ...
    #简单地递归调用了self.children()去处理自己及其子模块
    ...
    #将所有浮点类型的 parameters 和 buffer 转变成 float32 类型
    def float(self: T) -> T:
    ...
    #将所有浮点类型的 parameters 和 buffer 转变成 double 类型
    def double(self: T) -> T:
    ...
    #将所有浮点类型的 parameters 和 buffer 转变成 float16 类型
    def half(self: T) -> T:
    ...
    #将所有浮点类型的 parameters 和 buffer 转变成 bfloat16 类型
    ...
    #同样的，nn.Module 也支持注册只被应用于自己的 forward 和 backward hook，通过 3 个函数 来管理 自己的 3 个属性并维护 3 个 attribute，他们的类型也是 OrderedDict
    ...
    def _slow_forward(self, *input, **kwargs):
        ...
        return result
    #运行逻辑
    #self.__call__ ---> self._call_impl ---> 
    #_global_forward_pre_hooks --->self._forward_pre_hooks --->
    #self.forward/self.slow_forward --->
    #__global_forward_hooks --->self._forward_hooks --->
    #register_global_backward_hooks ---> register self._backward_hooks
    def _call_impl(self, *input, **kwargs):
        #同时我们注意到在 torch._C._get_tracing_state() 为 True 的时候，
    ...
    #5. 由第三点和前文所述的 _apply 实现可以得出 self.xxxx = torch.Tensor()是一种不被推荐的行为，
    # 因为这样新增的 attribute 既不属于 self._parameters，
    # 也不属于 self._buffers，而会被视为普通的 attribute ，
    # 在将模块进行状态转换的时候，
    # self.xxxx 会被遗漏进而导致 device 或者 type 不一样的 bug
    def __setattr__(self, name: str, value: Union[Tensor, 'Module']) -> None:
        ...
        if isinstance(value, Parameter):
            #检查继承nn.Module的自定义模块是否有正确初始化父类nn.Module, 
            #这也所有了super().__init__()的必要性
            if params is None:
                raise AttributeError(
                    "cannot assign parameters before Module.__init__() call")
            #在增加 self._parameters，self._modules 的时候，
            # 会预先调用 remove_from 函数 （15 和 29 行）
            # 从其余私有属性中删除对应的 name，这说明 self.dict，
            # self._buffers，self._parameters，self._modules 中的属性应该是互斥的
            ...
            if isinstance(value, Module):
                ...
                # __setattr__ 只能将 self._buffers 中已有的 buffer 重新赋值为 
                # None 或者 tensor 。
                # 这是因为 buffer 的初始化类型就是 torch.Tensor 或者 None，
                # 而不像 parameters 和 module 分别是 nn.Parameter 
                # 和 nn.Module 类型
    ...
    #使用 state_dict() 函数来进行获得当前的完整状态，用于在模型训练中储存 checkpoint
    ...
    #nn.Module 使用 load_state_dict() 函数来读取 checkpoint。
    #load_state_dict() 会通过调用每个子模块的_load_from_state_dict 函数来加载他们所需的权重，
    def load_state_dict(self, state_dict: 'OrderedDict[str, Tensor]',
        ...
        def load(module, prefix=''):
            ...
            #_load_from_state_dict 才是真正负责加载 parameter 和 buffer 的函数。这也说明了每个模块可以自行定义他们的 _load_from_state_dict 函数来满足特殊需求，实际上这也是 PyTorch 官方推荐的做法
    ...
    def _named_members(self, get_members_fn, prefix='', recurse=True):
        ...
        for module_prefix, module in modules:
            ...
            for k, v in members:
                ...
                yield name, v
    #常见的属性访问
    #他们都会返回一个迭代器用于访问模块中的 buffer，parameter，子模块等。

    #调用 self.named_parameters 并返回模型参数，被应用于 self.requires_grad_ 和 self.zero_grad 函数中
    def parameters(self, recurse: bool = True) -> Iterator[Parameter]:
        ...
        for name, param in self.named_parameters(recurse=recurse):
            yield param
    #返回 self._parameters 中的 name 和 parameter 元组，
    # 如果 recurse=True 还会返回子模块中的模型参数
    ...
    #返回 self._buffers 中的 name 和 buffer 元组，
    # 如果 recurse=True 还会返回子模块中的模型 buffer
    ...
    #调用 self.named_children，
    # 只返回 self._modules 中的模块，被应用于 self.train 函数中
    ...
    #只返回 self._modules 中的 name 和 module 元组
    def named_children(self) -> Iterator[Tuple[str, 'Module']]:
        ...
        for name, module in self._modules.items():
            if module is not None and module not in memo:
                memo.add(module)
                yield name, module
    #调用 self.named_modules 并返回各个 module 但不返回 name
    def modules(self) -> Iterator['Module']:
        ...
        for _, module in self.named_modules():
            yield module
    #返回 self._modules 下的 name 和 module 元组，并递归调用和返回 module.named_modules
    ...
    #nn.Module 重载了 __dir__ 函数，重载的 __dir__ 函数会将 self._modules、
    #self._parameters 和 self._buffers 中的 attributes 给暴露出来。
    def __dir__(self):
