...
#DP基于单机多卡，所有设备都负责计算和训练网络，除此之外，device[0]还要负责整合
#梯度，更新参数。
#3个主要过程：
# a.各卡分别计算损失和梯度;
# b.所有梯度整合到device[0];
# c.device[0]进行参数更新，其他卡拉取device[0]的参数进行更新
class DataParallel(Module):
    ...
    **scattered** on dim specified (default 0). tuple, list and dict types will
    ...
    def forward(self, *inputs, **kwargs):
        ...
        #运行前GPU device_ids[0](即我们的server)上必须有parallelized module 的parameters 和buffers
        #因为DP保证GPU device_ids[0] 和 base parallelized module 共享存储
        #所以device[0] 上的in-place更新也会被保存下来，其他的则不会
        ...
        #现在device[0]上有了module和input, 接下来我们就要开始PS算法了
