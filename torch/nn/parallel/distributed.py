...
#constructor
#   ×负责在构建的时候将rank0 的state_dict()广播-->保证所有网络初始状态相同
#   ×初始化bucket并尽可能按逆序将parameters分配进bucket-->按桶通信提高效率
#   x为每个parameter加上grad_accumulator以及在autograd_graph注册autograd_hook --> 在backward时负责梯度同步

#forward
#  × 正常的forward操作;
#  x 如果 self.find_unused_parameters 设置为 True，
#    DDP 会在 forward 结束时 traverse autograd graph 找到所有没用过的parameters 并标记为 ready ➜ 
#    虽说这一步开销很大，但是有时计算动态图会改变，所以很必要。

#autograd_hook
#  × 这个 hook 是挂在 autograd graph 在 backward 时负责梯度同步的。
#    当一个梯度计算好后，相应的 hook 会告诉 DDP 可以用来归约。
...
class DistributedDataParallel(Module):
    ...
    #负责多线程时复制模型、将parameters分组、创建reducer以及为SyncBN做准备
    def _ddp_init_helper(self, parameters, expect_sparse_gradient):
        """
        Initialization helper function that does the following:
        (1) bucketing the parameters for reductions（把 parameter 分组，梯度通讯时，先得到梯度的会通讯）
        (2) resetting the bucketing states
        (3) registering the grad hooks（创建管理器）
        ...
        #每个DDP进程都会创建本地Reducer在backward时管理梯度
    ...
    def forward(self, *inputs, **kwargs):
        ...
        if torch.is_grad_enabled() and self.require_backward_grad_sync:
            ...
            if self.find_unused_parameters:
                ...
                # 这样 backward 就可以在一个 subgraph 进行，但这样会牺牲一部分时间。
