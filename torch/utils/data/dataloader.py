...
#Dataloader负责总的调度，命令Sampler定义遍历索引的方式，然后用索引去Dataset中提取元素。
#是PyTorch数据加载的核心，负责加载数据，同时支持Map-Style和Iterable-style Dataset
#支持单进程/多进程，还可以设置loading order, batch_size, pin memory 等加载参数。
class DataLoader(Generic[T_co]):
    ...
    Args:
        dataset (Dataset): dataset from which to load the data.加载数据的数据集
        batch_size (int, optional): how many samples per batch to load
            (default: ``1``).
            每个batch加载多少个样本
        shuffle (bool, optional): set to ``True`` to have the data reshuffled
            at every epoch (default: ``False``).
            设置为True时，调用RandomSampler进行随机索引
        sampler (Sampler or Iterable, optional): defines the strategy to draw
            samples from the dataset. Can be any ``Iterable`` with ``__len__``
            implemented. If specified, :attr:`shuffle` must not be specified.
            定义从数据集中提取样本的策略，如果指定了，shuffle参数为False(否则会和RandomSampler互斥)
        batch_sampler (Sampler or Iterable, optional): like :attr:`sampler`, but
            returns a batch of indices at a time. Mutually exclusive with
            :attr:`batch_size`, :attr:`shuffle`, :attr:`sampler`,
            and :attr:`drop_last`.
            和sampler类似，但是一般传入BatchSampler,每次返回一个batch大小索引其和 batch_size, shuffle 等参数是互斥的
        num_workers (int, optional): how many subprocesses to use for data
            loading. ``0`` means that the data will be loaded in the main process.
            (default: ``0``)
            要用于数据加载的子进程数，0表示将在主进程中加载数据
        collate_fn (callable, optional): merges a list of samples to form a
            mini-batch of Tensor(s).  Used when using batched loading from a
            map-style dataset.
            在将 Map-style datase t 取出的数据整合成 batch 时使用，合并样本列表以形成一个 batch
        pin_memory (bool, optional): If ``True``, the data loader will copy Tensors
            into CUDA pinned memory before returning them.  If your data elements
            are a custom type, or your :attr:`collate_fn` returns a batch that is a custom type,
            see the example below.
            如果为 True，则 DataLoader 在将张量返回之前将其复制到 CUDA 固定的内存中
        drop_last (bool, optional): set to ``True`` to drop the last incomplete batch,
            if the dataset size is not divisible by the batch size. If ``False`` and
            the size of dataset is not divisible by the batch size, then the last batch
            will be smaller. (default: ``False``)
            设置为 True 删除最后一个不完整的批次，如果该数据集大小不能被该批次大小整除。
            如果 False 并且数据集的大小不能被批次大小整除，那么最后一批将较小
        timeout (numeric, optional): if positive, the timeout value for collecting a batch
            from workers. Should always be non-negative. (default: ``0``)
            如果为正，则为从 worker 收集 batch 的超时值，应始终为非负数超过这个时间还没读取到数据的话就会报错
        worker_init_fn (callable, optional): If not ``None``, this will be called on each
            worker subprocess with the worker id (an int in ``[0, num_workers - 1]``) as
            input, after seeding and before data loading. (default: ``None``)
            如果不为 None，它将会被每个 worker 子进程调用，以 worker id ([0, num_workers - 1] 内的整形) 为输入
        prefetch_factor (int, optional, keyword-only arg): Number of samples loaded
            in advance by each worker. ``2`` means there will be a total of
            2 * num_workers samples prefetched across all workers. (default: ``2``)
            每个 worker 提前加载 的 sample 数量
        persistent_workers (bool, optional): If ``True``, the data loader will not shutdown
            the worker processes after a dataset has been consumed once. This allows to
            maintain the workers `Dataset` instances alive. (default: ``False``)
            如果为 True，dataloader 将不会终止 worker 进程，直到 dataset 迭代完成
    ...
    #根据num_worker 获得迭代器，并指示进行单进程还是多进程
...
class _BaseDataLoaderIter(object):
    ...
    #dataloader提供了sampler(可以是batch_sampler或这其它sampler子类)，
    # 然后SingleProcessDataLoaderIter迭代sampler获得索引。
...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    def __init__(self, loader):
        ...
        #定义_dataset_fetcher, 并传入 _dataset, _auto_collation, _collate_fn 等参数，用于定义获取数据的方式
