...
#它是一种通过实现__getitem__() 和 __len()__来获取数据的Dataset, 
#它表示从（可能是非整数）索引/关键字到数据样本的映射.
class Dataset(Generic[T_co]):
    ...
    """
    #定义获取容器中指定元素时的行为，相当与self[key], 即允许类对象拥有索引操作
...
#它是一种实现 __iter__() 来获取数据的 Dataset，这种类型的数据集特别适用于以下情况：随机读取代价很大甚至不大可能，且 batch size 取决于获取的数据。
#特别地，当 DataLoader 的 num_workers > 0 时， 每个 worker 都将具有数据对象的不同样本。
# 因此需要独立地对每个副本进行配置，以防止每个 worker 产生的数据不重复。
# 同时，数据加载顺序完全由用户定义的可迭代样式控制。
# 这允许更容易地实现块读取和动态批次大小（例如，通过每次产生一个批次的样本）
...
#用于获取封装成tensor的数据集，每一个样本都通过索引张量来获得
